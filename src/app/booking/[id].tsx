...
export default function BookingDetailScreen() {
  ...
  if (!booking) {
    return <View style={styles.center}><Text style={{ color: colors.textMuted }}>Booking not found</Text></View>;
  }
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        {['finding_workers', 'bids_received'].includes(booking.status) ? (
          <View style={styles.card}>
            <View style={styles.cardHead}>
              <Ionicons name={workerBids.length === 0 ? 'radio-outline' : 'pricetags-outline'} size={18} color={colors.text} />
                  ...
                  <View key={bid._id} style={styles.bidCard}>
                    ...
                    {ns === 'worker_offered' && last ? (
                      <View style={styles.negNotice}>
                        <Ionicons name="chatbubble-ellipses-outline" size={13} color={colors.orangeDark} />
                        <Text style={styles.negHint}>Worker countered to {formatCurrency(last.amount)}</Text>
                      </View>
                    ) : ns === 'agreed' ? (
                      <View style={[styles.negNotice, { backgroundColor: colors.successBg }]}>
                        <Ionicons name="checkmark-circle" size={13} color={colors.success} />
                        <Text style={[styles.negHint, { color: colors.success }]}>Worker agreed to your price</Text>
                      </View>
                    ) : ns === 'customer_offered' ? (
                      <View style={styles.negNotice}>
                        <Ionicons name="time-outline" size={13} color={colors.orangeDark} />
                    ...
                    {isNeg ? (
                      <View style={styles.negInputRow}>
                        <View style={styles.amtInput}>
                          ...
                          <TextInput style={styles.amtField} value={counterAmount} onChangeText={setCounterAmount} keyboardType="number-pad" placeholder="Your offer" placeholderTextColor={colors.textLight} />
                        </View>
                        ...
                        <TouchableOpacity style={styles.cancelNeg} onPress={() => { setNegotiatingBidId(null); setCounterAmount(''); }}><Ionicons name="close" size={18} color={colors.textMuted} /></TouchableOpacity>
                      </View>
          ...
          <View style={styles.modalCard}>
            ...
            <View style={styles.modalBtns}>
              ...
              <TouchableOpacity style={[styles.confirmCancelBtn, busy && { opacity: 0.6 }]} onPress={doCancel} disabled={busy}>
                {busy ? <ActivityIndicator color={colors.white} /> : <Text style={styles.confirmCancelT}>Cancel Booking</Text>}
              </TouchableOpacity>
            </View>
          </View>
        </KeyboardAvoidingView>
...
const createStyles = (c: ThemeColors) =>
  ...
  amount: { fontSize: 24, fontWeight: '900', color: c.success, marginTop: 2 },
  ...
  schedCard: { flexDirection: 'row', alignItems: 'flex-start', gap: 10, backgroundColor: c.infoBg, borderWidth: 1, borderColor: c.info, borderRadius: 16, padding: 14 },
  schedTitle: { fontSize: 13.5, fontWeight: '800', color: c.info },
  schedSub: { fontSize: 11.5, color: c.info, marginTop: 3, lineHeight: 16 },
  ...
  liveDotInner: { height: 8, width: 8, borderRadius: 4, backgroundColor: c.success },
  liveText: { fontSize: 11.5, color: c.success, fontWeight: '700' },
  ...
  detailText: { fontSize: 14, color: c.textMuted, lineHeight: 20 },
  locRow: { flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 12 },
  locText: { fontSize: 13, color: c.textMuted, flex: 1 },
  busyCard: { flexDirection: 'row', alignItems: 'flex-start', gap: 12, backgroundColor: c.warnBg, borderRadius: 18, padding: 16, borderWidth: 1, borderColor: c.warn },
  busyIcon: { width: 34, height: 34, borderRadius: 17, backgroundColor: c.warnBg, alignItems: 'center', justifyContent: 'center' },
  ...
  ratingPill: { flexDirection: 'row', alignItems: 'center', gap: 3, backgroundColor: c.warnBg, paddingHorizontal: 9, paddingVertical: 5, borderRadius: 12 },
  ratingT: { fontSize: 12, fontWeight: '700', color: c.warn },
  waiting: { flexDirection: 'row', alignItems: 'center', gap: 10, paddingVertical: 8 },
  waitingText: { fontSize: 13, color: c.textMuted },
  bidCard: { padding: 14, borderRadius: 16, backgroundColor: c.bg, borderWidth: 1, borderColor: c.border, marginBottom: 10 },
  bidTop: { flexDirection: 'row', alignItems: 'center', gap: 12 },
  wAvatarSm: { height: 42, width: 42, borderRadius: 21, backgroundColor: c.navy50, alignItems: 'center', justifyContent: 'center' },
  wAvatarSmT: { color: c.text, fontWeight: '800', fontSize: 16 },
  bidName: { fontSize: 14.5, fontWeight: '800', color: c.text },
  bidRatingRow: { flexDirection: 'row', alignItems: 'center', gap: 3, marginTop: 2 },
  bidRating: { fontSize: 12, color: c.textMuted, fontWeight: '600' },
  bidPrice: { fontSize: 18, fontWeight: '900', color: c.success },
  bidMsg: { flexDirection: 'row', gap: 7, backgroundColor: c.bg, borderRadius: 10, paddingHorizontal: 11, paddingVertical: 9, marginTop: 10 },
  bidMsgT: { flex: 1, fontSize: 13, lineHeight: 18.5, color: c.text },
  negNotice: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: c.orange50, borderRadius: 10, paddingHorizontal: 10, paddingVertical: 8, marginTop: 10 },
  negHint: { fontSize: 12.5, color: c.orangeDark, fontWeight: '700', flex: 1 },
  bidActions: { flexDirection: 'row', gap: 8, marginTop: 12 },
  acceptBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, backgroundColor: c.success, borderRadius: 12, paddingVertical: 12 },
  acceptT: { color: c.white, fontSize: 13.5, fontWeight: '800' },
  negBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, backgroundColor: c.navy50, borderRadius: 12, paddingVertical: 12, paddingHorizontal: 16 },
  negBtnT: { color: c.text, fontSize: 13.5, fontWeight: '800' },
  negInputRow: { flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 12 },
  amtInput: { flex: 1, flexDirection: 'row', alignItems: 'center', backgroundColor: c.card, borderWidth: 1, borderColor: c.border, borderRadius: 12, paddingHorizontal: 12 },
  rupee: { fontSize: 15, fontWeight: '800', color: c.textMuted },
  amtField: { flex: 1, paddingVertical: 11, paddingLeft: 4, fontSize: 15, fontWeight: '700', color: c.text },
  sendBtn: { backgroundColor: c.orange, borderRadius: 12, paddingVertical: 12, paddingHorizontal: 18 },
  sendT: { color: c.white, fontSize: 13, fontWeight: '800' },
  cancelNeg: { padding: 8 },
  payAmountBox: { backgroundColor: c.successBg, borderRadius: 14, padding: 16, alignItems: 'center', marginBottom: 14 },
  ...
  pinText: { color: c.white, fontSize: 34, fontWeight: '900', letterSpacing: 8 },
  doneCard: { alignItems: 'center', gap: 4, backgroundColor: c.successBg, borderRadius: 18, padding: 24, borderWidth: 1, borderColor: c.success },
  doneIcon: { width: 64, height: 64, borderRadius: 22, backgroundColor: c.white, alignItems: 'center', justifyContent: 'center', marginBottom: 4 },
  doneText: { fontSize: 17, fontWeight: '800', color: c.success },
  thanksText: { fontSize: 12.5, color: c.textMuted, marginTop: 4 },
  waitRow: { flexDirection: 'row', alignItems: 'flex-start', gap: 8 },
  waitText: { flex: 1, fontSize: 13, color: c.textMuted, lineHeight: 18 },
  doneHead: { flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 14 },
  doneHeadText: { fontSize: 15, fontWeight: '800', color: c.text, flex: 1 },
  starsRow: { flexDirection: 'row', justifyContent: 'center', gap: 8, marginBottom: 14 },
  feedbackInput: { backgroundColor: c.bg, borderWidth: 1, borderColor: c.border, borderRadius: 12, padding: 14, fontSize: 14, color: c.text, height: 80, textAlignVertical: 'top' },
  submitReviewBtn: { backgroundColor: c.orange, borderRadius: 14, paddingVertical: 15, alignItems: 'center', marginTop: 12 },
  submitReviewT: { color: c.white, fontSize: 15, fontWeight: '800' },
  cancelBookingBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: c.dangerBg, borderRadius: 16, paddingVertical: 15, borderWidth: 1, borderColor: c.danger },
  cancelBookingT: { color: c.danger, fontSize: 14.5, fontWeight: '800' },
  modalBg: { flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' },
  modalCard: { backgroundColor: c.card, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24 },
  modalTitle: { fontSize: 18, fontWeight: '800', color: c.text },
  modalSub: { fontSize: 13, color: c.textMuted, marginTop: 4, marginBottom: 14 },
  reasonInput: { backgroundColor: c.bg, borderWidth: 1, borderColor: c.border, borderRadius: 12, padding: 14, fontSize: 14.5, color: c.text, height: 90, textAlignVertical: 'top' },
  modalBtns: { flexDirection: 'row', gap: 12, marginTop: 16 },
  keepBtn: { flex: 1, borderWidth: 1, borderColor: c.border, borderRadius: 12, paddingVertical: 14, alignItems: 'center' },
  keepT: { color: c.textMuted, fontWeight: '700' },
  confirmCancelBtn: { flex: 1, backgroundColor: c.danger, borderRadius: 12, paddingVertical: 14, alignItems: 'center' },
  confirmCancelT: { color: c.white, fontWeight: '800' },
});
