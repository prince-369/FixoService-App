...
export default function ChangePasswordScreen() {
  ...
  const getStrength = (pwd: string) => {
    if (!pwd) return { label: '', color: colors.textLight, score: 0 };
    let score = 0;
    ...
    return (
      <SafeAreaView style={styles.root} edges={['top']}>
        <View style={styles.topbar}>
          <TouchableOpacity onPress={() => { setForgotMode(false); setOtpSent(false); setResetToken(''); }}><Ionicons name="arrow-back" size={22} color={colors.text} /></TouchableOpacity>
        ...
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          {!otpSent && !resetToken && (
            <>
              ...
              <TextInput style={styles.input} value={forgotInput} onChangeText={setForgotInput} placeholder="Email ya phone number" placeholderTextColor={colors.textLight} autoCapitalize="none" keyboardType="email-address" />
              <TouchableOpacity style={[styles.btn, forgotBusy && styles.disabled]} onPress={sendForgotRequest} disabled={forgotBusy}>
                {forgotBusy ? <ActivityIndicator color={colors.white} /> : <Text style={styles.btnT}>Send Reset Request</Text>}
              </TouchableOpacity>
            </>
          )}
          {otpSent && !resetToken && (
            <>
              ...
              <TextInput style={styles.input} value={otp} onChangeText={setOtp} placeholder="Enter OTP" placeholderTextColor={colors.textLight} keyboardType="number-pad" maxLength={6} />
              <TouchableOpacity style={[styles.btn, forgotBusy && styles.disabled]} onPress={verifyOtp} disabled={forgotBusy}>
                {forgotBusy ? <ActivityIndicator color={colors.white} /> : <Text style={styles.btnT}>Verify OTP</Text>}
              </TouchableOpacity>
            </>
          )}
          {resetToken && (
            <>
              ...
              <View style={styles.pwdRow}>
                <TextInput style={[styles.input, { flex: 1 }]} value={resetPwd} onChangeText={setResetPwd} placeholder="New Password" placeholderTextColor={colors.textLight} secureTextEntry={!showResetPwd} />
                <TouchableOpacity onPress={() => setShowResetPwd(!showResetPwd)} style={styles.eye}><Ionicons name={showResetPwd ? 'eye-off' : 'eye'} size={20} color={colors.textMuted} /></TouchableOpacity>
              </View>
              {resetPwd ? <Text style={[styles.strength, { color: getStrength(resetPwd).color }]}>{getStrength(resetPwd).label}</Text> : null}
              <TextInput style={styles.input} value={resetConfirm} onChangeText={setResetConfirm} placeholder="Confirm New Password" placeholderTextColor={colors.textLight} secureTextEntry />
              <TouchableOpacity style={[styles.btn, forgotBusy && styles.disabled]} onPress={doReset} disabled={forgotBusy}>
                {forgotBusy ? <ActivityIndicator color={colors.white} /> : <Text style={styles.btnT}>Reset Password</Text>}
              </TouchableOpacity>
            </>
          )}
        </ScrollView>
  ...
  return (
    <SafeAreaView style={styles.root} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.pwdRow}>
          <TextInput style={[styles.input, { flex: 1 }, currentError && styles.inputErr]} value={currentPwd} onChangeText={(v) => { setCurrentPwd(v); setCurrentError(''); }} placeholder="Enter current password" placeholderTextColor={colors.textLight} secureTextEntry={!showCurrent} />
          <TouchableOpacity onPress={() => setShowCurrent(!showCurrent)} style={styles.eye}><Ionicons name={showCurrent ? 'eye-off' : 'eye'} size={20} color={colors.textMuted} /></TouchableOpacity>
        </View>
        ...
        <View style={styles.pwdRow}>
          <TextInput style={[styles.input, { flex: 1 }]} value={newPwd} onChangeText={setNewPwd} placeholder="Enter new password" placeholderTextColor={colors.textLight} secureTextEntry={!showNew} />
        ...
        <View style={styles.pwdRow}>
          <TextInput style={[styles.input, { flex: 1 }]} value={confirmPwd} onChangeText={setConfirmPwd} placeholder="Re-enter new password" placeholderTextColor={colors.textLight} secureTextEntry={!showConfirm} />
          <TouchableOpacity onPress={() => setShowConfirm(!showConfirm)} style={styles.eye}><Ionicons name={showConfirm ? 'eye-off' : 'eye'} size={20} color={colors.textMuted} /></TouchableOpacity>
        </View>
        ...
        <TouchableOpacity style={[styles.btn, busy && styles.disabled]} onPress={handleChangePassword} disabled={busy}>
          {busy ? <ActivityIndicator color={colors.white} /> : <Text style={styles.btnT}>Update Password</Text>}
        </TouchableOpacity>
      </ScrollView>
    </SafeAreaView>
  );
...
const createStyles = (c: ThemeColors) =>
  ...
  desc: { fontSize: 13.5, color: c.textMuted, lineHeight: 20, marginBottom: 16 },
  label: { fontSize: 12, fontWeight: '700', color: c.textMuted, marginTop: 16, marginBottom: 6, textTransform: 'uppercase' },
  input: { backgroundColor: c.card, borderWidth: 1, borderColor: c.border, borderRadius: 12, paddingHorizontal: 14, paddingVertical: 13, fontSize: 15, color: c.text },
  inputErr: { borderColor: c.danger },
  pwdRow: { flexDirection: 'row', alignItems: 'center', gap: 0 },
  eye: { position: 'absolute', right: 12, padding: 4 },
  errT: { fontSize: 12, color: c.danger, marginTop: 4, fontWeight: '600' },
  strength: { fontSize: 12, fontWeight: '700', marginTop: 4 },
  forgotLink: { alignSelf: 'flex-end', marginTop: 8 },
  forgotT: { fontSize: 13, fontWeight: '700', color: c.orange },
  btn: { backgroundColor: c.orange, borderRadius: 12, paddingVertical: 15, alignItems: 'center', marginTop: 24 },
  btnT: { color: c.white, fontSize: 15, fontWeight: '800' },
